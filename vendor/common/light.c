...
const light_res_hw_t light_res_hw[LIGHT_CNT][5] = {
	/*[0] = */{RES_HW_PWM_R, RES_HW_PWM_G, RES_HW_PWM_B, RES_HW_PWM_W, RES_HW_PWM_Y}, // vc can't use "[0]="
};
...
void light_dim_refresh(int idx) // idx: index of LIGHT_CNT.
{
    ...
    light_dim_set_hw(idx, 2, get_pwm_cmp(0xff, (100-mi_ct)*lum_100/100));
            #else
    if(ct_flag){
        light_dim_set_hw(idx, 3, get_pwm_cmp(0xff,lum_100));
        light_dim_set_hw(idx, 4, get_pwm_cmp(0xff, ct_100));
        light_dim_set_hw(idx, 0, get_pwm_cmp(0xff,0));
        light_dim_set_hw(idx, 1, get_pwm_cmp(0xff, 0));
        light_dim_set_hw(idx, 2, get_pwm_cmp(0xff, 0));
    }
            #endif   
        #endif
    ...
    if(!ct_flag){
    	...
    	light_dim_set_hw(idx, 3, get_pwm_cmp(0xff, 0));
    	light_dim_set_hw(idx, 4, get_pwm_cmp(0xff, 0));
        ...
        HslToRgb(HSL,&RGB);
        light_dim_set_hw(idx, 0, get_pwm_cmp(0xff, RGB.b*100/255));
        light_dim_set_hw(idx, 1, get_pwm_cmp(0xff, RGB.g*100/255));
        light_dim_set_hw(idx, 2, get_pwm_cmp(0xff, RGB.r*100/255));
        //static u32 A_1[100];
        // static u32 A_1_cnt;
        //if(A_1_cnt < 100){
